console.log("connecter");const container = document.querySelector(".container");
...
// Nous initialisons les variables qui vont changer d'état.
...
const createCarrousel = () => {
  ...
  const degress = 360 / length; // Notes pour chaque élément
  ...
  container.style.width = tz * 4 + gap * length + "px";
...
// Fonction qui donne de la douceur à l'animation.
const lerp = (a, b, n) => {
  return n * (a - b) + b;
};
...
const distanceZ = (widthElement, length, gap) => {
  return (widthElement / 1) / Math.tan(Math.PI / length) + gap; // Distancia Z de los items
}

// Calculez la hauteur du conteneur en utilisant le champ de vision et la distance de perspective.(taille)
const calculateHeight = z => {
  const t = Math.atan(10 * Math.PI / 100 /0.5);
  const height = t * 2.5 * z;

  return height;
};

// Calculer le champ de vision du carrousel
...
const onResize = () => {
  // Obtient les propriétés de la taille du carrousel
